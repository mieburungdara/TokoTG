...
class Template {
  ...
  public function page_loader() {
    if ($this->page_loader) {
        echo '<div id="page-loader" class="show"></div>';
    }
  }
  ...
  /**
     ...
     */
    public function html_classes($print = true) {
      // Build <html> classes
      if ($this->remember_theme) {
          $this->html_classes .= ' remember-theme';
      }

      // Print or return <html> classes
      if ($this->html_classes) {
          if ($print) {
              echo ' class="'. trim($this->html_classes) .'"';
          } else {
              return trim($this->html_classes);
          }
      } else {
          return false;
      }
...
";
        $this->nav_html .= "$link_icon$link_name$link_badge</a>\n";
...
";
  ...
  public function get_css($asset_css) {
    echo "<link rel=\"stylesheet\" href=\"" . base_url($this->assets_folder . "/" . $asset_css) . "\">
";
  }
  ...
  public function get_js($asset_js) {
    echo "<script src=\"" . base_url($this->assets_folder . "/" . $asset_js) . "\"></script>\n";
  }
  ...
  public function get_avatar($id = 0, $gender = '', $size = 0, $thumb = false, $class = '') {
    ...
    echo "<img class=\"img-avatar" . $class_f . "\" src=\"" . base_url($this->assets_folder . "/media/avatars/avatar" . $id_f . ".jpg") . "\" alt=\"\">
";
  }
  ...
  public function get_photo($id = 0, $retina = false, $class = '') {
    echo "<img" . ($class ? ' class="' . $class . '"' : '') . " src=\"" . base_url($this->assets_folder . "/media/photos/photo" . ($id ? $id : rand(1, 25)) . ($retina ? '@2x' : '') . ".jpg") . "\" alt=\"\">
";
  }
  ...
  public function get_tag($print = true, $status = false) {
    ...
    $tag = '<span class="badge bg-' . $data[$rand]['class'] . '">' . $data[$rand]['text'] . '</span>' . "\n";
